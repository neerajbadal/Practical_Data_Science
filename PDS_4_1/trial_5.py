...
if __name__ == "__main__":
    ...
    model_df = pd.read_csv(format_dir+"/param_form_test_v4.dat",sep='\t',header=None)
    print(model_df)
    ...
    for model_inst in model_df:
        ...
        mod = sm.tsa.SARIMAX(countryDataFrame['log_Confirmed'][:cut_off[country_index]], maxiter=50,order=simple_order,
        ...
        plt.title(model_names[i_count]+"  "+str(mse_)+"  "+str(np.sqrt(mse_))+
                  "  "+str(mse_test)+"  "+str(np.sqrt(mse_test)))
