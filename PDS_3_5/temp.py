...
#     def giveBestSplit(self,split_val,colIndex):
    def giveBestSplit(self,lhs_indices,rhs_indices):
#         print(lhs_indices)
...
#                 probable_split[2] = rhs_indices
        ...
        sortedIndex = np.argsort(self.xvals[:,colIndex])
        
#         print(self.xvals.shape,".....",sortedIndex.shape)
#         print(sortedIndex)
        sortedXVals = self.xvals[sortedIndex]
        sortedGradient = self.gradient_[sortedIndex]
        sortedHessian = self.hessian_[sortedIndex]
        
#         np.take_along_axis(self.xvals,sortedIndex, axis=0)
        
        lhs_cut = np.searchsorted(sortedXVals[:,colIndex],quantiles_,'right')
        
        
        grad_hess = [ [np.sum(sortedGradient[:lhs_cut_quant]),
                       np.sum(sortedGradient[lhs_cut_quant:]),
                       np.sum(sortedHessian[:lhs_cut_quant]),
                       np.sum(sortedHessian[lhs_cut_quant:])  ]
               for lhs_cut_quant in lhs_cut]
        
        
        
        gain_ = (grad_hess[0]**2) / (grad_hess[2] + self.lambda_)
        gain_ += (grad_hess[1]**2) / (grad_hess[3] + self.lambda_)
        gain_ -= (grad_hess[0] + grad_hess[1])**2 / (grad_hess[2]+grad_hess[3]+self.lambda_)
        gain_ -= self.gamma_
        gain_ = gain_ / 2.0
...
#         
#         gainResults = [self.giveBestSplit(lhs_indices_bucket[buck_ind],rhs_indices_bucket[buck_ind]) for buck_ind in range(0,len(lhs_indices_bucket))]
        
        gainResults = np.array(gain_)
